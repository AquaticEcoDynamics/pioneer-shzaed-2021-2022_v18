"""
6-panel (2 x 3) animation of nitrogen-related 2D sheet diagnostics from
aed_data_cmb_*.nc. All variables are element-centred sheet quantities.

Layout (left-to-right, top-to-bottom):
  Row 1:  NIT_amm_dsf   NIT_nit_dsf   NIT_din_atm
  Row 2:  OGM_don_swi   OGM_pon_swi   OGM_pon_res

Same panel size/aspect/xlim/ylim/colourbar treatment as the 3-panel scripts;
just stacked 2 rows. The top-left panel carries the Dry % inset; the other
five do not.

The variables here are all stored as (time, nface) sheet fields. The script's
existing element-centred path handles them naturally (layer_dim=None,
shading="flat" with the tri_to_elem mapping).

Usage:
  python animate_Nfluxes.py --n-stacks 1                  # quick test
  python animate_Nfluxes.py --n-stacks 14 --inset-horizon-days 3
"""
...
DEFAULT_OUT = Path(__file__).parent / "Nfluxes_animation.mp4"


# 6 panels, 3 columns × 2 rows. Iteration order: row-major (left-to-right,
# top-to-bottom). build_animation places panels into the flattened grid in
# the same order.
PANEL_VARS = [
    "NIT_amm_dsf",  "NIT_nit_dsf",  "NIT_din_atm",
    "OGM_don_swi",  "OGM_pon_swi",  "OGM_pon_res",
]
...
def main():
    ...
    p.add_argument("--cmap", default="berlin",
                   help="Colormap for all six flux panels (default: berlin via cmcrameri).")
